...
envelope.addEventListener("click",()=>{
  ...
  setTimeout(()=>{intro.classList.add("hide");document.getElementById("top").scrollIntoView({behavior:"smooth"});tryMusic(); confetti();},1100);
});
function tryMusic(){music.play().then(()=>musicBtn.innerHTML="♫ <span>On</span>").catch(()=>musicBtn.innerHTML="♫ <span>Music</span>")}
musicBtn.addEventListener("click",()=>{if(music.paused){music.play();musicBtn.innerHTML="♫ <span>On</span>";musicBtn.classList.add("active");}else{music.pause();musicBtn.innerHTML="♫ <span>Music</span>";musicBtn.classList.remove("active");}});

document.getElementById("startBtn").addEventListener("click",()=>document.getElementById("memories").scrollIntoView({behavior:"smooth"}));
...
candles.forEach(c=>c.addEventListener("click",()=>{c.classList.add("off");c.textContent="♡";let left=candles.filter(x=>!x.classList.contains("off")).length;if(left)status.textContent=`${left} little flame${left>1?"s":""} left... make it a good one ✨`;else{status.textContent="Wish released into the universe. 💗";for(let i=0;i<3;i++)setTimeout(()=>confetti(),i*400)}}));
window.addEventListener("keydown",e=>{
  if(e.key==="Escape")document.getElementById("lightbox").classList.remove("open");
});
const colors=["#f28caf","#f4b44f","#9b59b6","#2ecc71"];
p.style.color=colors[Math.floor(Math.random()*colors.length)];
document.getElementById("againBtn").addEventListener("click",()=>{
  window.scrollTo({top:0,behavior:"smooth"});
  location.reload(); // optional, but smoother reset is better
});
